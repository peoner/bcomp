...
public class Processor {
	public Processor(int[] A, int[] CK, int[] KR, int[] RA, int[] RC, int[] RD, int[] RK, int[] Memory, int[][] MemoryMK)
	{
		...
		if (GetParam(0, A.length, A) == 0) { this.N[0] = 1; this.RC[11] = 1; }
		else if (A[0] == 1) { this.Z[0] = 1; this.RC[10] = 1; }
	}
	public static int GetParam(int x0, int x1, int[] binaryMK)
	{
		String s_param = new String();
		for (int i = x0; i < x1; ++i)
			s_param += System.Convert.ToString(binaryMK[i]);
		return System.Convert.ToInt32(s_param, 2);
	}
	public static int[] GetParam(int[] reg0, int[] reg1, int[] reg2, int[] reg3,
		int x0, int x1, int[] binaryMK)
	{
		return binaryMK[x0] * binaryMK[x1] == 1 ? reg0 : binaryMK[x0] + binaryMK[x1] == 0 ?
		reg1:binaryMK[x0] == 1 ? reg2 : reg3;
	}
	public static int[] MinimazeSumm(int[] i_right, int iright, int length, int dopl1, int dopl2, int dopl3)
	{
		if (length == 16)
			return iright < dopl1 ? Program.ConvertToBit("000" + System.Convert.ToString(iright, 16), 0) :
			iright < dopl2 ? Program.ConvertToBit("00" + System.Convert.ToString(iright, 16), 0) :
			iright < dopl3 ?Program.ConvertToBit("0" + System.Convert.ToString(iright, 16), 0):
			Program.ConvertToBit(System.Convert.ToString(iright, 16), 0);

		else if (length < 16)
		{
			return length < dopl2 ? Program.ConvertToBit("00" + System.Convert.ToString(iright, 16), 0) :
				length < dopl3 ? Program.ConvertToBit("0" + System.Convert.ToString(iright, 16), 0) :
				Program.ConvertToBit(System.Convert.ToString(iright, 16), 0);
				}
		else if (length > 16) return Program.ConvertToBit(System.Convert.ToString(iright, 16), 3);
		return null;
	}
	...
	public static class ALY { 
	    ...
	    public int[] SummRegister(boolean ifOpen) 
		{
			...
			for (int i = ch_summ.length - 1, j = i_summ.length - 1; i > -1 && j > -1; --i, --j)
				i_summ[j] = System.Convert.ToInt32(ch_summ[i]) - 48;
			return i_summ;
		}
	...
	public static class YY
	{
		public static void DecodeOMK0(int[] binaryMK, int[] mem)
		{
			...
			{
				int i_RA = GetParam(0, 11, RA);
				int i_Mem = System.Convert.ToInt32(System.Convert.ToString(mem[i_RA]));
				System.Console.WriteLine(Integer.toHexString(i_Mem).ToUpper() + "->ÐÄ");
				RD = MinimazeSumm(mem, i_Mem, 16, 16, 16*16, 16*16*16);
			}

				int i_result = GetParam(0, 17, alu.bufregister);
				System.Console.WriteLine(Integer.toHexString(i_result).ToUpper() + "->ÁÐ");
		}
		public static void ExitBR(int[] register, int what/*òîò, ÷òî â ÀËÓ*/)
		{
		...
		public static void DecodeOMK1(int[] binaryMK)
		{
			...
			{ C[0] = 0;  RC[12] = 0; } 
			else if (binaryMK[8] == 1 && binaryMK[9] == 1) 
			{ C[0] = 1; RC[12] = 1; }
			else if (binaryMK[10] == 1)
			{
				if (A[0] == 1) { N[0] = 1; RC[11] = 1; }
			}
			else if (binaryMK[11] == 1) { 
				int Ak = GetParam(0, 16, A); if (Ak == 0)
			{ Z[0] = 1; RC[10] = 1; } } 
			else if (binaryMK[12] == 1) HLT = true;
		...
		public static void DecodeUMK(int[] binaryMK)
		{
			...
			System.Console.WriteLine(Integer.toHexString(GetParam(0, 17, ALY.bufregister)).ToUpper() + "->ÁÐ");
			
			if (register[register.length - numbBit - 1] == binaryMK[1])
			{
				System.Console.WriteLine(Integer.toHexString(addrPereh).ToUpper() + "->C÷ÌÊ");
				ChMK = addrPereh;
			}
			else
				++ChMK;
	...
	static int[] A, CK, RA, RC, RD, RK, KR,/* bufreg, */N = new int[1], Z = new int[1], C = new int[1], Memory;
	static int[][] MemoryMK;
}
...
public class Program
{
	public static int[] ConvertToBit(String buffer, int what)
	{
		...
		int length = what == 0 ? 16 : what == 1 ? 12 : 17;//17 ïðè 3

		int[] i_binaryMK = new int[length];

		for (int z = 0; z < ch_buffer.length; ++z)
		{
			mk = System.Convert.ToInt32(System.Convert.ToString(ch_buffer[z]), 16); //16 - âñå ïèøåì â øåñòíàäöàòåðè÷íîé 
	...
	public static Processor Read()
	{
		...
		for (int i = 4; i < 7; ++i)
		{
			...
			int AddrInMem = System.Convert.ToInt32(System.Convert.ToString(mas[i][0]), 16);
			int operand = System.Convert.ToInt32(System.Convert.ToString(mas[i][1].Trim(new char[] { '\r' })), 16);
		...
		{
			mas[i] = s_plit[i].Split(new char[] { '\t' });
			int[] MicroCommand = ConvertToBit(System.Convert.ToString(mas[i][0].Trim(new char[] { '\r' })), 0);
			for (int j = 0; j < 16; ++j) MemoryMK[i - 6][j] = MicroCommand[j];
		}
		return new Processor(A, CK, KR, RA, RC, RD, RK, /*bufreg,*/ Memory, MemoryMK);
	}
	public static void main(String[] args)
	{
		...
		do
		{
			...
			writer.WriteLine("ÐÌÊ " + s_RMK + "   |ÑÊ " + s_CK + "   |ÐÀ " + s_RA +
				"   |ÐÊ " + s_RK + "   |ÐÄ " + s_RD + "   |À " + s_A + "   |Ñ÷ÌÊ " + s_ChMK
				+ "   |ÁÐ " + Integer.toHexString(i_BR).ToUpper());
